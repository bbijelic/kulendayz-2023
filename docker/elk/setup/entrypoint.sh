...
log "Checking for defined policies"

for policy in "${BASH_SOURCE[0]%/*}/policies"/*; do

    if [[ ! -f "${policy}" ]]; then
		sublog 'No policies defined, skipping'
		continue
	fi
    ...
    sublog "Handling policy: $policy_name"

    create_ilm_policy "$policy_name" "$policy_body"
done

log "Checking for defined index templates"

for template in "${BASH_SOURCE[0]%/*}/templates"/*; do

    if [[ ! -f "${template}" ]]; then
		sublog 'No index templates defined, skipping'
		continue
	fi
    ...
    sublog "Handling index template: $template_name"
